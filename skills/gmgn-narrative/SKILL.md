...
Two of these nine — `arbitrum` and `hyperevm` — are recent additions and behave slightly differently for a couple of on-chain fields (notably around LP-lock/burn reporting, documented in `gmgn-contract-dd`'s own skill file). Nothing in this skill's own fields (`symbol`, `name`, the `dev.*` and `link.*` fields, `fee_distribution`, `launchpad` / `launchpad_platform`, and the optional security fields) is currently known to need chain-specific handling — but this hasn't been specifically stress-tested on those two chains yet, only confirmed to return data at all. Treat that as untested rather than confirmed-fine until a few real tokens on each have actually been run through this skill.

...
| `gmgn-contract-dd` | Scored 0-100 contract due-diligence verdict — contract safety + holder structure + price action, with per-field deductions, weight renormalisation over the sections that returned data, and a reported coverage that bounds the claim | User gives a contract address and wants a **verdict or score** rather than raw fields — "CA 尽调", "这个币能不能买", "评个分", "is this contract safe, score it", "due diligence on this token". `gmgn-token` hands scoring off to this skill |
...
- **`skills/`** — 12 skill definitions for Claude Code

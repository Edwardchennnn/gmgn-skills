...
| `gmgn-wallet-analysis` | The decision dossier on a wallet — four pass/fail gates (is the record real, is the edge still working this week, can you get filled, does it cut losses), the live book, entry market-cap band, copy window and a size cap | **The default for a bare wallet address**, and for any "should I act on this wallet" question: 「这个钱包能跟吗」, 「帮我分析一下这个钱包」, 「它现在在买什么」, 「这个钱包最近还行吗」, "can I copy this wallet", "analyze this wallet", "is this wallet still hot" |
| `gmgn-holder-analysis` | Token holder chip analysis — chip distribution, entry cost, whale/dev/KOL behaviour, risk wallets (rat traders, bundlers, snipers), related wallets | User asks 筹码分析, 持仓分析, chip structure, who is holding, or whether a token is safe to buy based on holder composition |
...
| "analyze this wallet", "is this wallet worth following", 「这个钱包能跟吗」, a bare wallet address | `gmgn-wallet-analysis` — it owns the verdict. Reach for `gmgn-portfolio` only when the user wants the raw holdings / activity / stats rather than a judgement |
...
| "跟单评分", "钱包评分", "copy trade score" — an explicit ask for a **number** | `gmgn-wallet-score` (copy-tradeability angle — score + latency/slippage/gas backtest) |
| "值不值得跟单", "is this wallet worth copying" — an ask for a **decision** | `gmgn-wallet-analysis`. Two skills must not both answer this: the dossier owns the verdict, the score is the numeric annex |
| "钱包发盘情况怎么样", "是不是发币方钱包", "dev 信誉怎么样", "is this a token-creator wallet" | `gmgn-wallet-score` (Dev-reputation angle) |

**One question, one owner.** `gmgn-wallet-analysis` and `gmgn-wallet-score` read the same portfolio endpoints and must never be offered as alternatives for the same question — they contradicted each other on a live BSC wallet (`0x1e48…2aab`), one printing 「能看不能抄」 while the other scored copy-tradeability 81/100. The dossier decides; the score quantifies what the dossier already decided.
